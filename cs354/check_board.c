...
int valid_board(int **board, int size) {
	...
	for (int i = 0; i < size; i++){
		...
		for (int j = 0; j < size; j++) {
			...
			// only check if the content is not 0
			if (*(*(board + i) + j) != 0) {
			
				// check element after the current element at the same row for duplicates
				for (int row = 1; row < (size - i); row++) {
					if ( *(*(board + i) + j) == *(*(board + i + row) + j) ) {
						
						// debug_info
						printf("At row %i, column %i\n", i + 1, j + 1);
						printf("row checking, checking  number = %i against %i\n", *(*(board + i) + j), *(*(board + i + row) + j));
				...
				for (int column = 1; column < (size - j); column++) {
					if ( *(*(board + i) + j) == *(*(board + i) + j + column) ) {
						...
						// debug_info
						printf("At row %i, column %i\n", i + 1, j + 1);
						printf("column checking, checking number = %i against %i\n", *(*(board + i) + j), *(*(board + i) + j + column));
...
int main( int argc, char **argv ) {              
	...
	// allocate a 1D array of integer points with a size of the board
	...
	// allocate the each row for a 1D array of integers with a size of the board
